...
class Naskar2021(LinearCouplingModel):
    ...
    t_glu = Attr(default=7.46, attr_type=AttrType.Model)    # concentration of glutamate
    t_gaba = Attr(default=1.82, attr_type=AttrType.Model)   # concentration of GABA
    We = Attr(default=1.0, attr_type=AttrType.Model)        # scaling of external input current to excitatory population
    Wi = Attr(default=0.7, attr_type=AttrType.Model)        # scaling of external input current to inhibitory population
    I0 = Attr(default=0.382, attr_type=AttrType.Model)      #.397  # [nA] overall effective external input
    w = Attr(default=1.4, attr_type=AttrType.Model)         # weight for recurrent self-excitation in each excitatory population
    J_NMDA = Attr(default=0.15, attr_type=AttrType.Model)   # [nA] NMDA current
    M_e = Attr(default=1.0, attr_type=AttrType.Model)
    ae = Attr(default=310.0, attr_type=AttrType.Model)      # [nC^{-1}], g_E in the paper
    be = Attr(default=125.0, attr_type=AttrType.Model)      # = g_E * I^{(E)_{thr}} in the paper = 310 * .403 [nA] = 124.93
    de = Attr(default=0.16, attr_type=AttrType.Model)
    ai = Attr(default=615.0, attr_type=AttrType.Model)      # [nC^{-1}], g_I in the paper
    bi = Attr(default=177.0, attr_type=AttrType.Model)      # = g_I * I^{(I)_{thr}} in the paper = 615 * .288 [nA] = 177.12
    di = Attr(default=0.087, attr_type=AttrType.Model)
    M_i = Attr(default=1.0, attr_type=AttrType.Model)
    alfa_e = Attr(default=0.072, attr_type=AttrType.Model)  # forward rate constant for NMDA gating
    alfa_i = Attr(default=0.53, attr_type=AttrType.Model)   # forward rate constant for GABA gating
    B_e = Attr(default=0.0066, attr_type=AttrType.Model)    # ms^-1  backward rate constant for NMDA gating
    B_i = Attr(default=0.18, attr_type=AttrType.Model)      # ms^-1  backward rate constant for GABA gating
    gamma = Attr(default=1.0, attr_type=AttrType.Model)     # Learning rate
    rho = Attr(default=3.0, attr_type=AttrType.Model)       # target-firing rate of the excitatory population is maintained at the 3 Hz
